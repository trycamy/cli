...
sign in, inspect, sign out

### Examples

```
camy auth login                  one click in your browser; the key lands in your keychain
camy auth status                 who you are and what the key may do
camy auth logout --revoke        remove the key here and kill it server-side
```
...
* [camy auth login](camy_auth_login.md)	 - Sign in from the browser — one click there, an expiring key here
* [camy auth logout](camy_auth_logout.md)	 - Remove the local key; --revoke also kills it server-side
* [camy auth status](camy_auth_status.md)	 - Identity, key prefix, scopes, api_url
