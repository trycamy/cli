...
who can open a site — public today; passcode and unlisted are not available yet

```
camy canvas access set SITE MODE [flags]
```

### Options

```
      --clear-passcode   remove the stored passcode (mode=public)
  -h, --help             help for set
      --passcode-stdin   read the passcode from stdin (scripts)
```
