...
Press one of the card's buttons

### Synopsis

Press one of the card's buttons.

Behavior change: this confirms like every other mutating verb — a TTY
asks y/N, and a headless run exits 2 without --force, including a
--json caller. Script it with --force once you trust the action_id,
the same way you already do for `camy canvas restore` or
`camy canvas sites rm`.

```
camy feed act ID ACTION_ID [flags]
```
...
```
  -h, --help          help for act
      --note string   optional note to send with the action
```
