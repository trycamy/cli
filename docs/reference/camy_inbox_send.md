...
Compose and send a brand-new email — synchronous, no outbox, no undo

```
camy inbox send TO... [flags]
```
...
```
      --at string         schedule instead of sending now: a duration (2h) or RFC3339
  ...
  -h, --help              help for send
      --provider string   gmail|outlook (default: first connected)
