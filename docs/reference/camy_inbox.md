...
unified inbox — list, show, reply, undo
...
```
camy inbox                       the unified inbox, needs-you first
camy inbox --needs-you           only what's waiting on you
camy inbox show em_7f31          one email with its triage
camy inbox reply em_7f31         the grounded draft; --send queues it with an undo window
```
