...
what's happening right now — one glance
...
```
camy status                      the glance — approvals, mail, jobs, workspace, credits
camy status --watch              keep it live
```
...
```
  -h, --help    help for status
      --watch   refresh every 15s
  -w, --web     open camy.ai instead
```
