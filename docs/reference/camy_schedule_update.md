## camy schedule update

Change a scheduled task's cron, timezone or delivery channels

```
camy schedule update ID [flags]
```

### Examples

```
  camy schedule update ab12cd34 --cron "0 8 * * *"
  camy schedule update ab12cd34 --tz America/Chicago
  camy schedule update ab12cd34 --channels thread,email
```

### Options

```
      --channels string   comma-separated: thread,email,inbox,push,user_email
      --cron string       new 5-field cron (e.g. '0 8 * * *')
  -h, --help              help for update
      --tz string         new IANA timezone
```
