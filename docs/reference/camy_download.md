...
Save a chat attachment to disk — the handle every 📎 receipt prints

```
camy download ATTACHMENT_ID [flags]
```

### Examples

```
  camy download 2f1c…-…-…   # name from the server
  camy download 2f1c…-…-… -o report.pdf
```
...
```
  ...
  -o, --output string   destination path (default: the server's filename)
```
