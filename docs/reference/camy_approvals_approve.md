...
Approve a checkpoint — the paused turn resumes; --wait streams it here

```
camy approvals approve ID... [flags]
```

### Examples

```
  camy approvals approve ap_789a
  camy approvals approve ap_789a ap_2c40   # both, one line each
  camy approvals approve ap_789a --wait    # --wait takes one id
```

### Options

```
      --chat string   chat to attach with --wait
  -h, --help          help for approve
      --wait          attach and stream the resumed turn to completion
```
