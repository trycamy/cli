...
your cloud workspace — exec, shell, status
...
```
camy vm status                   is the workspace up, and what it costs
camy vm exec -- pytest -q        run a command there; its exit code becomes yours
camy vm shell                    a live shell
```
