...
Pull a queued send back before it leaves

```
camy inbox undo OUTBOX_ID... [flags]
```

### Examples

```
  camy inbox outbox                     # the queue, with its ob_ ids
  camy inbox undo ob_31f2               # inside the undo window
  camy inbox undo ob_31f2 ob_9c4d       # pull both back
```
