...
- Per platform, a tarball — `camy_<version>_<os>_<arch>.tar.gz`, holding the binary and, when the release ships them, shell completions and man pages — and a CycloneDX SBOM for it. From 1.0.2 the macOS binaries are code-signed with Apple Developer ID and notarized.
