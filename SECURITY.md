...
- Per release, `SHA256SUMS`, plus a version-scoped checksum manifest with a cosign signature and certificate from keyless signing, and a minisign signature.
...
Neither the installer nor `camy update` checks the cosign or minisign
signature; they trust TLS plus the checksums.
[Verifying releases](docs/verifying-releases.md) has the `cosign verify-blob`
recipe, the channel layout, and how to read the SBOM. The minisign public key is published in
